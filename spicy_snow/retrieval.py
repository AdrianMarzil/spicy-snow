...
def retrieve_snow_depth(area: shapely.geometry.Polygon, 
    ...
    if check_s1_downloaded(search_results, tmp_dir):
        ...
        msg = (
            f"Sentinel-1 cache incomplete in: {tmp_dir}\n"
            f"This run will SUBMIT and DOWNLOAD new Sentinel-1 RTC products via ASF HyP3.\n"
            f"This may consume HyP3 quota.\n"
            f"Continue? [y/N]: "
        )
        resp = input(msg).strip().lower()

        if resp not in ("y", "yes"):
            raise RuntimeError("[STOP] User chose not to submit HyP3 jobs. No Sentinel-1 downloads were started.")
    ...
    ALTERNATE CODE - Add a STOP if there a no images (prevents Hyp3 download)
        tmp_dir = join(work_dir, "tmp")
